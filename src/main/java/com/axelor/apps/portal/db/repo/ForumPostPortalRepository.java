...
public class ForumPostPortalRepository extends ForumPostRepository {
  @Override
  public ForumPost save(ForumPost post) {
    ...
    if (post.getVersion() == 0
        && post.getForumGroup() != null
        && post.getForumGroup().getWorkspace() != null) {
      Beans.get(NotificationService.class)
          .notifyUser(
              "forum",
              post.getForumGroup().getId(),
              post.getClass().getName(),
              post.getForumGroup().getWorkspace());
    }
